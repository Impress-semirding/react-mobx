...
const QiniuImageUpload = React.createClass({
  ...
  render:function(){
    var previewImageShow = this.state.previewImageShow&&(this.props.mode === undefined||this.props.mode==="dialog")?"":"hide";
    var progressHide = this.state.isPicUploading?"":"hide";
    var uploadContent = (
      <div>
        <div className="row">
          Select Upload File:
          <input ref="uploadSelector" type="file" onChange={this.previewImage} id="onlyImage" />
        </div>
  ...
  doImageUpload:function(){
    this.props.uploadToken({
      success:function(data){
        var token = data;
        Qiniu_upload(React.findDOMNode(this.self.refs.uploadSelector).files[0],token.token,null,{
          success:function(data){
...
export default QiniuImageUpload;
